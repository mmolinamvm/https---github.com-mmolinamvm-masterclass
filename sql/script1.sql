...
DROP USER IF EXISTS 'myguiamanu'@'localhost';

-- Creem l'usuari amb autenticació clàssica per contrasenya (Nivell DWES)
CREATE USER 'myguiamanu'@'localhost' IDENTIFIED BY '@Asustek6247';

-- 2. GESTIÓ DE LA BASE DE DADES
DROP DATABASE IF EXISTS guiamanudb;
CREATE DATABASE guiamanudb CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Assignem tots els privilegis a l'usuari web ÚNICAMENT sobre aquesta base de dades
GRANT ALL PRIVILEGES ON guiamanudb.* TO 'myguiamanu'@'localhost';
FLUSH PRIVILEGES;
...
CREATE TABLE IF NOT EXISTS mc_videos (
...
ALTER TABLE mc_preguntes ADD COLUMN video_id INT AFTER id;

-- 4. Assignem totes les teves preguntes actuals al vídeo 1
UPDATE mc_preguntes SET video_id = 1;

-- 5. Fem la columna FK NOT NULL i creem la relació formal
ALTER TABLE mc_preguntes MODIFY COLUMN video_id INT NOT NULL;
ALTER TABLE mc_preguntes ADD CONSTRAINT fk_preguntes_videos FOREIGN KEY (video_id) REFERENCES mc_videos(id) ON DELETE CASCADE;
